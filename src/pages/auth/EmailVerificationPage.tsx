...
export default function EmailVerificationPage() {
  ...
  const handleResend = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // In a real app, call the API to resend the email verification code
      // await ApiService.auth.sendEmailOtp({ email });
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      const response: any = await ApiService.auth.verifyEmail({ email, code: otpCode });
      if (response && response.success && response.user && response.token) {
        login(response.user, response.token);
        refreshUser();
        router.push('/profile');
      } else {
        router.push('/auth/login?message=' + encodeURIComponent('Email verified. Please log in.'));
      }
    } catch (err: any) {
